...
-- Includes: NIN verification, biometric auth, email verification.
-- ============================================================
...
CREATE TABLE users (
    ...
    biometric_credentials        JSONB,                   -- WebAuthn credential objects
    pending_webauthn_challenge   VARCHAR(255),
    pending_challenge_expires    TIMESTAMP,
    biometric_enrolled_at        TIMESTAMP,
    require_biometric_for_voting BOOLEAN  DEFAULT FALSE,

    -- ── NIN (National ID) verification ─────────────────────────────────────
    -- Never store the raw NIN — only the SHA-256 hash
    nin_hash                    VARCHAR(64) UNIQUE,        -- SHA-256(NIN + salt)
    nin_verified                BOOLEAN     DEFAULT FALSE,
    nin_verified_at             TIMESTAMP,
    nin_verification_reference  VARCHAR(255),              -- Korapay reference ID
    nin_first_name              VARCHAR(100),
    nin_last_name               VARCHAR(100),
    nin_middle_name             VARCHAR(100),
    nin_gender                  VARCHAR(10),
    nin_state_of_origin         VARCHAR(50),
    nin_photo_match_score       INTEGER,                   -- 0-100 confidence
    nin_data_matched            BOOLEAN     DEFAULT FALSE,
    nin_photo_matched           BOOLEAN     DEFAULT FALSE
);
...
CREATE TABLE elections (
    ...
    require_biometric        BOOLEAN      DEFAULT FALSE,   -- election-level biometric gate
...
CREATE TABLE votes (
    ...
    -- 🆔 NIN verification snapshot (state at time of vote)
    nin_verified               BOOLEAN DEFAULT FALSE,
    nin_verification_reference VARCHAR(255),
    ...
    verification_method     VARCHAR(50) DEFAULT 'nin_password',
...
CREATE INDEX idx_votes_nin       ON votes(nin_verified);
...
INSERT INTO system_configuration (config_key, config_value, config_type, description) VALUES
    ('app_name',                  'Intelligent Voting System', 'string',  'Application name'),
    ('allow_registration',        'true',                      'boolean', 'Allow new user registration'),
    ('require_email_verification','true',                      'boolean', 'Require email verification before login'),
...
INSERT INTO ml_model_configuration
    (model_name, model_type, is_active, parameters, threshold_config, feature_weights)
VALUES (
...
-- NOTE: regenerate this hash in production with bcrypt.generate_password_hash('yourpassword')
INSERT INTO users (
    voter_id, full_name, email, password_hash, role,
    is_active,
    email_verified, email_verified_at,
    nin_verified                          -- admin bypasses NIN
) VALUES (
    'ADMIN001', 'System Administrator', 'admin@voting.com',
...
-- Sample voter — email verified, NIN NOT verified (to test the NIN gate)
INSERT INTO users (
    voter_id, full_name, email, password_hash, role,
    is_active,
    email_verified, email_verified_at,
    ...
    'VID001', 'John Doe', 'voter@test.com',
    '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5zzDKJ/cDKMgy',
    'voter',
    true,
    true, NOW(),
    false   -- ← intentionally unverified so you can test the NIN gate
);

-- Sample voter — fully verified (to test the happy path)
INSERT INTO users (
    voter_id, full_name, email, password_hash, role,
    is_active,
    email_verified, email_verified_at,
    nin_verified, nin_verified_at,
    nin_data_matched, nin_photo_matched,
    nin_hash
) VALUES (
    'VID002', 'Jane Smith', 'verified@test.com',
    '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5zzDKJ/cDKMgy',
    'voter',
    true,
    true, NOW(),
    true, NOW(),
    true, false,
    'aabbccddeeff00112233445566778899aabbccddeeff00112233445566778899' -- fake hash for seeding only
);
...
-- Positions
INSERT INTO positions (election_id, title, description, display_order) VALUES
    (1, 'President',      'Student Union President',      1),
    (1, 'Vice President', 'Student Union Vice President', 2),
    (1, 'Secretary',      'Student Union Secretary',      3),
    (1, 'Treasurer',      'Student Union Treasurer',      4);

-- Candidates
INSERT INTO candidates (position_id, name, party, biography, image_url, display_order) VALUES
    (1, 'Sarah Johnson',  'Progressive Alliance', 'Experienced leader with vision for change',    '👩‍💼', 1),
    (1, 'Michael Chen',   'Reform Party',         'Committed to transparency and innovation',     '👨‍💼', 2),
    (2, 'Amara Okafor',   'Unity Front',          'Dedicated to student welfare',                 '👩‍⚖️', 1),
    (2, 'David Martinez', 'Change Coalition',     'Advocate for student rights',                  '👨‍🏫', 2),
    (3, 'Emily Brown',    'Progressive Alliance', 'Detail-oriented administrator',                '👩‍💻', 1),
    (3, 'James Wilson',   'Reform Party',         'Organised and efficient',                      '👨‍💻', 2),
    (4, 'Lisa Anderson',  'Unity Front',          'Financial management expert',                  '👩‍💼', 1),
    (4, 'Robert Taylor',  'Change Coalition',     'Experienced in budget planning',               '👨‍💼', 2);
